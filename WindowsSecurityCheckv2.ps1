...
$scores   = @{}
$script:severityWeights = @{ OK = 0; WARN = 2; CRIT = 5 }
$script:maxSeverityWeight = $script:severityWeights["CRIT"]
...
function Add-Finding {
    ...
    if (-not $script:scores.ContainsKey($category)) { $script:scores[$category] = @{Lost=0;Max=0;Scored=0} }
    if ($script:severityWeights.ContainsKey($status)) {
        $script:scores[$category].Lost += $script:severityWeights[$status]
        $script:scores[$category].Max += $script:maxSeverityWeight
        $script:scores[$category].Scored++
    }
}
...
$lostSeverityPoints = ($totalWarn * $script:severityWeights["WARN"]) + ($totalCrit * $script:severityWeights["CRIT"])
$maxSeverityPoints = $totalChecks * $script:maxSeverityWeight

$overallScore = 0
if ($maxSeverityPoints -gt 0) {
    $raw = 100 - [math]::Round(($lostSeverityPoints / $maxSeverityPoints) * 100)
...
$scoreQuality = "Bad score"
if ($overallScore -ge 90) { $scoreQuality = "Strong score" }
elseif ($overallScore -ge 75) { $scoreQuality = "Good score" }
elseif ($overallScore -ge 60) { $scoreQuality = "Medium score" }
elseif ($overallScore -ge 40) { $scoreQuality = "Weak score" }

if ($maxSeverityPoints -gt 0) {
    $scoreReason = "all scored checks passed."
    if ($totalCrit -gt 0) { $scoreReason = "$totalCrit critical finding(s) have the highest weight." }
    elseif ($totalWarn -gt 0) { $scoreReason = "$totalWarn warning finding(s) reduce the score." }
    $scoreSummary = "$scoreQuality. $scoreReason $totalGood/$totalChecks scored checks passed. Formula: 100 - round($lostSeverityPoints/$maxSeverityPoints * 100) = $overallScore."
} else {
    $scoreSummary = "No scored checks ran, so BeSecured cannot rate this machine yet."
}
$scoreWeightsText = "OK = 0, WARN = $($script:severityWeights['WARN']), CRIT = $($script:severityWeights['CRIT']) severity points lost. INFO is not scored."
...
# Chart data, rendered with local CSS only
$categoryBarsHtml = [System.Text.StringBuilder]::new()
foreach ($catName in ($scores.Keys | Sort-Object)) {
    $s = $scores[$catName]
    if ($s.Max -gt 0) {
        $pct = 100 - [math]::Round(($s.Lost / $s.Max) * 100)
        $pct = [math]::Max(0, [math]::Min(100, $pct))
        [void]$categoryBarsHtml.AppendLine("<div class='bar-row'><div class='bar-label'>$catName</div><div class='bar-track'><span style='width:$pct%'></span></div><div class='bar-value'>$pct%</div></div>")
    } else {
        [void]$categoryBarsHtml.AppendLine("<div class='bar-row'><div class='bar-label'>$catName</div><div class='bar-track'><span style='width:0%'></span></div><div class='bar-value'>N/A</div></div>")
    }
}

$impactCats = @(
    $scores.GetEnumerator() |
        Where-Object { $_.Value.Lost -gt 0 } |
        Sort-Object @{Expression={ $_.Value.Lost }; Descending=$true}, @{Expression={ $_.Key }; Ascending=$true} |
        Select-Object -First 3
)
$impactText = "No warning or critical finding removed points."
if ($impactCats.Count -gt 0) {
    $impactParts = @()
    foreach ($item in $impactCats) { $impactParts += "$($item.Key) ($($item.Value.Lost))" }
    $impactText = "Main score impact: " + ($impactParts -join ", ") + "."
}
...
body{background:var(--bg);color:var(--text);font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;min-height:100vh;padding:2rem}
...
.logo{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:.75rem;color:var(--accent);letter-spacing:4px;text-transform:uppercase;margin-bottom:.5rem}
...
.subtitle{color:var(--muted);font-size:.9rem;margin-top:.3rem;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}
...
.card-title{font-size:.7rem;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:var(--muted);margin-bottom:1rem;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}
.privacy{margin-bottom:1.5rem;color:var(--text)}
.privacy p{color:var(--muted);font-size:.9rem}
.score-formula{color:var(--text)!important;font-weight:650}
...
.si-key{color:var(--muted);padding:.35rem 0;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;width:40%}
.si-val{color:var(--text);font-weight:600}
.chart-wrap{display:grid;gap:.8rem;min-height:260px;align-content:center}
.bar-row{display:grid;grid-template-columns:140px 1fr 48px;gap:.75rem;align-items:center}
.bar-label{color:var(--text);font-size:.8rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
.bar-value{color:var(--muted);font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:.8rem;text-align:right}
.findings-table{width:100%;border-collapse:collapse;font-size:.83rem}
.findings-table thead th{background:var(--surface2);color:var(--muted);text-transform:uppercase;letter-spacing:2px;font-size:.68rem;padding:.7rem .8rem;text-align:left;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;border-bottom:1px solid var(--border)}
...
.cat{color:var(--accent);font-weight:600;font-size:.78rem;white-space:nowrap;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}
.detail{color:var(--muted);font-size:.8rem}
.footer{text-align:center;margin-top:2rem;color:var(--muted);font-size:.75rem;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}
</style></head><body>')
...
[void]$sb.AppendLine('<div class="card privacy"><div class="card-title">Privacy</div><p>BeSecured runs 100% locally. No scan data is sent to a server. No account, cloud API or remote backend is used.</p></div>')
[void]$sb.AppendLine("<div class='card privacy'><div class='card-title'>Score Explanation</div><p class='score-formula'>$scoreSummary</p><p>$scoreWeightsText</p><p>$impactText</p></div>")
...
[void]$sb.AppendLine("<div class='card'><div class='card-title'>Score by Category</div><div class='chart-wrap'>$($categoryBarsHtml.ToString())</div></div>")
[void]$sb.AppendLine("<div class='card'><div class='card-title'>Finding Distribution</div><div class='chart-wrap'>$distributionBarsHtml</div></div>")
[void]$sb.AppendLine('</div>')
